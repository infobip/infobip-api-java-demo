...
public class DeliveryReportAdapter {
     ...
     * <a href="https://dev.infobip.com/docs/delivery-reports">documentation</a>.
     */
    private final GetSentSmsDeliveryReports deliveryReportsClient;

    /**
     * @param deliveryReportsClient client that the adapter uses for retrieving delivery reports from the Infobip API
     */
     ...
     * that request and will return only when response from the API is received.
     *
     * @param messageId of the SMS message which delivery report is to be retrieved
    ...
    public DeliveryReport getDeliveryReportWithId(String messageId) {
        try {
            /*
            Delivery reports can be retrieved by specifying either bulkId or messageId. The bulkId is used for getting
            reports of multiple messages sent at once in a single API request. The messageId is used for getting a
            single report of one SMS message sent by itself in one API request.
            As this application ony sends single SMSes, delivery reports are retrieved by messageId. The last parameter
            specifies the upper limit of reports that are to be returned by the API. Since in this case reports are
            retrieved one by one, limit is also set to one.
             */
            ...
            report objects. In this case, delivery report for only a single message was requested, so only the first
            element of that list is of interest. If no delivery reports for a given messageId are found, the following
            line results in an IndexOutOfBoundsException being thrown. That exception is caught and processed at the
         ...
         */
            ...
            /*
            By default a ReportFetchingException exception is thrown, with RetrofitError instance passed on as a cause.
            ...
            /*
            In case no delivery report was found a dedicated exception type is thrown. This is because a delivery
